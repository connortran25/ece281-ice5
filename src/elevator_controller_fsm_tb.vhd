...
begin
	...
	begin
		...
		w_reset <= '0'; w_stop <= '1'; wait for k_clk_period;
		    assert w_floor = x"2" report "bad reset clear" severity failure;
		-- active UP signal
		w_up_down <= '1';
		    assert w_floor = x"2" report "bad reset clear" severity failure;
		-- go up a floor
        ...
        wait for k_clk_period*3;
            assert w_floor = x"4" report "breaches ceiling" severity failure;
        -- go all the way down DOWN (how many clock cycles should that take?)
        w_up_down <= '0'; wait for k_clk_period*3;
            assert w_floor = x"1" report "bad down x3 from floor4" severity failure;
